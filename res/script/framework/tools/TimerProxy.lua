...
function TimerProxy:ctor()
	...
	local timer_init_, timer_delay_, timer_running_
	...
	timer_delay_ = function(timer, dt)
		...
		if timer._elapseDelay > 0.0 then return end
		timer._status = TIMER_STATUS.RUNNING
		timer._elapse = timer.interval
	end
	...
	local function timer_update_(timer, dt)
		...
		elseif timer._status == TIMER_STATUS.DELAY then
			return timer_delay_(timer, dt)
		elseif timer._status == TIMER_STATUS.RUNNING then
			return timer_running_(timer, dt)
		elseif timer._status == TIMER_STATUS.FINISH then
			return
		end
